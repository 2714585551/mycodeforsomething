...
public class SysDepartController {
	...
	public ResponseEntity<?> delete(final HttpServletRequest request, final HttpServletResponse response) {
		...
		sysDepartDetailService.deleteByDepartID(list);
	...
	public ResponseEntity<?> deleteDetail(final HttpServletRequest request, final HttpServletResponse response) {
		List<Long> list = ParamUtil.buildDeleteContidion(request);
		sysDepartDetailService.deleteByPks(list);
		ResponseMessage message = new ResponseMessage(true, "删除成功");
		return new ResponseEntity<>(message, HttpStatus.OK);
	}
	...
	@RequestMapping("/	send")
	@ResponseBody
	public ResponseEntity<?> send(final HttpServletRequest request, final HttpServletResponse response) {
		String departID = request.getParameter("id");
		SysDepartService.updateLoadStatus(Long.valueOf(departID));
		ResponseMessage message = new ResponseMessage(true, "发车成功");
		return new ResponseEntity<>(message, HttpStatus.OK);
	}
