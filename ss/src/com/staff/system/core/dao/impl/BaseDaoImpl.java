 ...
 * 
 */
package com.staff.system.core.dao.impl;
...
import com.staff.system.core.enums.QuerySqlTypeEnum;
...
import com.staff.system.core.vo.QueryPlaceholder;


/**
 * @ClassName: BaseDaoImpl
 * @FullClassPath: com.lpp.mq.dao.impl.BaseDaoImpl
 * @Description: Dao封装接口实现.
 * @author: Arno
 * @date: 2017年3月4日 下午6:02:38
 * @version: 1.0
 * @param <T>
 * @param <PK>
 */
...
public  class BaseDaoImpl<T, PK extends Serializable> implements BaseDao<T, PK> {
	...
	@Override
	public void saveBatch(List<T> entitys, Integer flushCount) {
	...
	@Override
	public PageData<T> findPageByFetchedHql(String hql, String countHql, int pageNo, int pageSize, Object... values) {
	...
	private List<QueryPlaceholder> buildQueryConditons(StringBuilder builder, QueryPage queryPage) {
		...
		if (!params.isEmpty()) {
			for (QueryParam p : params) {
				...
				if (StringUtils.isBlank(field) || null == value) {
					continue;
				}
				...
				if (logic.equalsIgnoreCase(QuerySqlTypeEnum.IN.getType())) {
					builder.append(SymbolConstats.SPACE + QuerySqlTypeEnum.AND.getType() + SymbolConstats.SPACE + field
							+ SymbolConstats.SPACE + logic + SymbolConstats.lEFT_BRACE + placeholder
							+ SymbolConstats.RIGHT_BRACE + SymbolConstats.SPACE);
				} else {
					builder.append(SymbolConstats.SPACE + QuerySqlTypeEnum.AND.getType() + SymbolConstats.SPACE
							+ SymbolConstats.SPACE + field + SymbolConstats.SPACE + logic + placeholder
		...
		if (!holders.isEmpty()) {
			for (QueryPlaceholder holder : holders) {
				...
				if (operator.equalsIgnoreCase(QuerySqlTypeEnum.IN.getType())) {
					query.setParameterList(holder.getPlaceholder(), (Collection<?>) holder.getValue());
				} else if (operator.equalsIgnoreCase(QuerySqlTypeEnum.LIKE.getType())) {
					query.setParameter(holder.getPlaceholder(),
							SymbolConstats.SIGN + holder.getValue().toString() + SymbolConstats.SIGN);
				} else {
					query.setParameter(holder.getPlaceholder(), holder.getValue());
				}
			}
		}
	}
